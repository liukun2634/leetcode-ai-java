...
### 面试官常见 follow-up
1. **"只需判是否有环（不说入环点）？"** → [LC 141](https://leetcode.cn/problems/linked-list-cycle/)，第一阶段就够。
2. **"在数组中找重复数（不能修改数组、O(1) 空间）？"** → 把数组看作“index → value”的函数迭代 = 链表路径，Floyd 找入环点。即 [**LC 287**](https://leetcode.cn/problems/find-the-duplicate-number/)。
...
### 同类型推荐（**快慢指针家族**）
- [LC 141. 环形链表](https://leetcode.cn/problems/linked-list-cycle/)（只判存在）
- [LC 287. 寻找重复数](https://leetcode.cn/problems/find-the-duplicate-number/)（**数组转链表**，Floyd 神级应用）
- [LC 19. 删除链表的倒数第 N 个结点](https://leetcode.cn/problems/remove-nth-node-from-end-of-list/)（快慢指针差距 N）
...
- [LC 202. 快乐数](https://leetcode.cn/problems/happy-number/)（Floyd 判循环）
