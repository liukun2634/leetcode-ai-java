...
> **学习点 ②**：拓扑排序天然给出了一种 **合法学习顺序**（[LC 210](https://leetcode.cn/problems/course-schedule-ii/) 直接返回这个顺序）。
...
### 面试官常见 follow-up
1. **"返回一个合法学习顺序？"** → 记录出队顺序即可。即 [**LC 210**](https://leetcode.cn/problems/course-schedule-ii/)。
2. **"起点 / 终点给定，求从起点还能抵达哪些节点？"** → 从起点 DFS/BFS，收集可达集合。
3. **"有环时返回环上的节点？"** → DFS 三色时遇到灰色节点记录状态以重构环。
4. **"火星词典（从排序后的单词推出字母顺序）？"** → 两两比较单词建边，拓扑。即 [**LC 269**](https://leetcode.cn/problems/alien-dictionary/)。
5. **"拓扑顺序唯一性判定（是否只有一种）？"** → Kahn 每轮队列只有一个节点 → 唯一。即 **LC 444**。
6. **"边有周期限制、计算最少学期数（拓扑分层）？"** → Kahn 按层处理，层数 = 学期数。即 [**LC 1136**](https://leetcode.cn/problems/parallel-courses/)。

### 同类型推荐（**拓扑排序家族**）
- [LC 210. 课程表 II](https://leetcode.cn/problems/course-schedule-ii/)（返回学习顺序）
- [LC 269. 火星词典](https://leetcode.cn/problems/alien-dictionary/)（拓扑排序应用）
