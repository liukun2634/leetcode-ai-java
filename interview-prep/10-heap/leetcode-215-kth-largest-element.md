...
### 面试官常见 follow-up
1. **"数据流中求第 K 大？"** → 小顶堆保持大小 K，addNum O(log K)。即 [**LC 703**](https://leetcode.cn/problems/kth-largest-element-in-a-stream/)。
2. **"前 K 高频元素？"** → 哈希计数 + 小顶堆按频次；或桶排序 O(n)。即 [**LC 347**](https://leetcode.cn/problems/top-k-frequent-elements/)。
3. **"最接近原点的 K 个点？"** → 大顶堆按距离平方，堆大于 K 则弹顶。即 [**LC 973**](https://leetcode.cn/problems/k-closest-points-to-origin/)。
4. **"多个数组合并后求第 K 小？"** → 小顶堆装 (值, 数组编号, 下标)，类 [LC 23](https://leetcode.cn/problems/merge-k-sorted-lists/) 合并。
5. **"有序矩阵中第 K 小？"** → 小顶堆 BFS；或二分答案。即 [**LC 378**](https://leetcode.cn/problems/kth-smallest-element-in-a-sorted-matrix/)。
6. **"如果允许修改数组，要返回前 K 大下标集合？"** → 快选后取右半部分；不要求顺序。

### 同类型推荐（**TopK / 堆家族**）
- [LC 703. 数据流中的第 K 大元素](https://leetcode.cn/problems/kth-largest-element-in-a-stream/)（小顶堆经典）
- [LC 692. 前 K 个高频单词](https://leetcode.cn/problems/top-k-frequent-words/)
- [LC 347. 前 K 个高频元素](https://leetcode.cn/problems/top-k-frequent-elements/)（桶排序 / 堆）
- [LC 295. 数据流的中位数](https://leetcode.cn/problems/find-median-from-data-stream/)（**双堆** 必刷）
- [LC 23. 合并 K 个升序链表](https://leetcode.cn/problems/merge-k-sorted-lists/)（堆）
- [LC 378. 有序矩阵中第 K 小的元素](https://leetcode.cn/problems/kth-smallest-element-in-a-sorted-matrix/)（堆 / 二分）
