...
### 面试官常见 follow-up
1. **"求二叉树直径（路径节点数最大）？"** → 同模板，返回贡献是边数；全局 max(left+right)。即 [**LC 543**](https://leetcode.cn/problems/diameter-of-binary-tree/)。
2. **"最长同值路径（路径上节点值相同）？"** → 同模板加一个“子节点与父同值才能拼接”的条件。即 **LC 687**。
3. **"二叉树偌间里最大金额不互相打劫（[LC 337](https://leetcode.cn/problems/house-robber-iii/)）？"** → 返回双状态 `(偊, 不偊)`，同是树形 DP。
...
### 同类型推荐（**树形 DP 家族**，全是高频）
- [LC 543. 二叉树的直径](https://leetcode.cn/problems/diameter-of-binary-tree/)（返回贡献是"边数"，全局是"两边相加"）
- LC 687. 最长同值路径
- LC 1372. 二叉树中的最长交错路径
- LC 968. 监控二叉树（树形 DP + 状态机）
- [LC 337. 打家劫舍 III](https://leetcode.cn/problems/house-robber-iii/)（"偷 / 不偷"双状态返回）
- [LC 110. 平衡二叉树](https://leetcode.cn/problems/balanced-binary-tree/)（返回高度，全局判平衡）
- [LC 236. 最近公共祖先](https://leetcode.cn/problems/lowest-common-ancestor-of-a-binary-tree/)（递归返回"子树是否含 p/q"）
